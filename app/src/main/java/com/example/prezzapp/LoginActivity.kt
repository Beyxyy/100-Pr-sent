        ...
        Thread {
            ...
            if (userDao.getAll().isEmpty()) {
                ...
                val presenceList = listOfNotNull(
                    coursAmine.find { it.prof == "Joel Dion" }?.let {
                        Presence(0, amine.id, it.id, estJustifie = false, lien = null, estPresent = false)
                    },
                    coursAmine.find { it.prof == "Luc Bernard" }?.let {
                        Presence(0, amine.id, it.id, estJustifie = false, lien = null, estPresent = false)
                    },
                    coursAmine.find { it.prof == "Julie Petit" }?.let {
                        Presence(0, amine.id, it.id, estJustifie = true, lien = "justificatif.pdf", estPresent = false)
                    }
                )
